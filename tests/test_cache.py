...
async def test_watch_cmd():
    entry = PortEntry(user="abc", port=11203, internal_url="internal", external_url="external")
    metric = CacheMetrics(time=123, producers=1, recvd=2,
                          sent=1, buffered=1)
    ...
    if False:
        metric2 = CacheMetrics(time=124, producers=0, recvd=2,
                           sent=2, buffered=0)
        entry = PortEntry(user="abc", port=11203,
                          internal_url="internal",
                          external_url="external")
        await watch_cmd("bash", "-c", "echo '%s'; sleep 1; echo '%s'"%(
                        metric.model_dump_json(),
                        metric2.model_dump_json()),
                    port=entry)
...
async def test_cache_job_complete(unused_tcp_port_factory, config):
    ...
    entry = PortEntry(user="abc", port=port1,
    ...
    await entry.transition(ClientName.producer,
                           JobState.new,
                           jobndx = 0,
                           info = "",
                           job = job)
