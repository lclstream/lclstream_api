...
def _snapshot(
    ...
    *,
    ...
    last_polled_at: datetime | None = NOW,
) -> TransferSnapshot:
...
def check_decision(
    ...
    *,
    ...
    decision = decide_state_with_timeout(
        ...
        max_ready_stale_s=NEVER_TIMEOUT,
    )
...
def test_provisioning_timeout_forces_failure() -> None:
    decision = decide_state_with_timeout(
        TransferObservation(
            cache_state=CacheState.queued, producer_state=JobState.QUEUED
        ),
        _snapshot(TransferState.provisioning),
...
def test_ready_staleness_forces_failure() -> None:
    decision = decide_state_with_timeout(
        TransferObservation(
            cache_state=CacheState.active, producer_state=JobState.ACTIVE
        ),
        ...
        now=NOW + timedelta(seconds=61),
        max_provisioning_age_s=NEVER_TIMEOUT,
        max_ready_stale_s=60.0,
    )
    assert decision.state is TransferState.failed
    assert decision.source is TransitionSource.orchestrator
    assert (
        decision.reason
        == "ready state stale for too long without a successful observation"
    )


def test_ready_within_stale_window_is_not_failed() -> None:
    decision = decide_state_with_timeout(
        TransferObservation(
            cache_state=CacheState.active, producer_state=JobState.ACTIVE
        ),
...
def test_ready_staleness_does_not_fire_without_a_prior_observation() -> None:
    ...
    decision = decide_state_with_timeout(
        ...
        _snapshot(TransferState.ready, last_polled_at=None),
        now=NOW + timedelta(seconds=10_000),
        max_provisioning_age_s=NEVER_TIMEOUT,
        max_ready_stale_s=60.0,
    )
...
def test_ready_staleness_yields_to_a_real_signal() -> None:
    ...
    decision = decide_state_with_timeout(
        TransferObservation(
            cache_state=CacheState.active, producer_state=JobState.FAILED
        ),
        _snapshot(TransferState.ready),
        now=NOW + timedelta(seconds=10_000),
        max_provisioning_age_s=NEVER_TIMEOUT,
    ...
    assert decision.state is TransferState.failed
    assert decision.source is TransitionSource.producer
    assert decision.reason is None
