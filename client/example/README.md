...
change without rebuilding anything of ours. Arguments come from the submit
command and land last on the indexamajig line, after the ones this data path
requires (`--zmq-input`, `--peaks=msgpack`, `--data-format=msgpack`, the
`--copy-header`s, `-g`, `--temp-dir`, `-o`):

```sh
uv run submit_crystfel_nersc.py --exp mfx101592326 --run 12 \
    --crystfel-args "--indexing=mosflm -j 8 --int-radius=3,4,5"
# or after a bare --
...
it is not baked into any image.
