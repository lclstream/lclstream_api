...
Perlmutter node against the S3DF cache, then xrdcp's the stream back.
    ...
    uv run submit_crystfel_nersc.py --exp ... --run ... -- --indexing=mosflm -j 8
...
def log_file(cfg: Settings, exp: str, run: str) -> Path:
    return cfg.directory / f"crystfel.{exp}.{run}.log"


def token_file(cfg: Settings, exp: str, run: str) -> Path:
    """Per job: the job deletes it, and a shared name would race."""
    return cfg.directory / ".secrets" / f"s3df_token.{exp}.{run}"
...
def stage_token(cfg: Settings, facility: FacilityClient, exp: str, run: str) -> Path:
    """Upload the S3DF token the job needs to create its transfer.

    Keyed per job: a shared path would let a queued job read a later
    submitter's credential and charge the transfer to them.
    """
    try:
...
def submit(
    ...
    token = stage_token(cfg, facility, exp, run)
    log = log_file(cfg, exp, run)
    job = facility.resource(cfg.compute_resource).submit(
        executable="/bin/bash",
        # The job body exports this as the source identifier.
        # Anything after the token is passed straight to indexamajig.
...
def main() -> int:
    ...
    parser.add_argument(
        "--crystfel-args", default="", help="extra indexamajig arguments, quoted"
    )
    parser.add_argument(
        "rest", nargs="*", help="extra indexamajig arguments, after --"
    )
